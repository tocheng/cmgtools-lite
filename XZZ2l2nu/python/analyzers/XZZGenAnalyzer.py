...
class XZZGenAnalyzer( Analyzer ):
    ...
    def makeMCInfo(self, event, bosonID = 23):
        ...
        pruned = self.mchandles['genParticles'].product()

        event.genParticles = [ p for p in pruned ]
        event.genZBosons = []
        event.genLeptons = []
	...
	event.genTausFsr = []
        ...
        if self.cfg_ana.verbose:
            print "event.genIncomingQuarks: "+str(len(event.genIncomingQuarks))
        ...
        if self.cfg_ana.verbose:
            print "event.genMuons: "+str(len(event.genMuons))
        ...
        if self.cfg_ana.verbose:
            print "event.genElectrons: "+str(len(event.genElectrons))
