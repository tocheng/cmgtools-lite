...
sigMassWindow = {
'BulkGravToZZToZlepZinv_narrow_600' : [60,200,700],
...
for idx,sample in enumerate(sigSamples):
    ...
    h1.append(sigPlotters[idx].drawTH1(sample+'_hmt','llnunu_mt',cuts,str(lumi*1000),nbins,xmin,xmax,titlex = "M_{T}",units = "GeV"))
    h2.append(sigPlotters[idx].drawTH1(sample+'_hmta','llnunu_mta',cuts,str(lumi*1000),nbins,xmin,xmax,titlex = "M_{T}",units = "GeV"))
    h3.append(sigPlotters[idx].drawTH1(sample+'_hmtb','llnunu_mtb',cuts,str(lumi*1000),nbins,xmin,xmax,titlex = "M_{T}",units = "GeV"))
    h4.append(sigPlotters[idx].drawTH1(sample+'_hmtc','llnunu_mtc',cuts,str(lumi*1000),nbins,xmin,xmax,titlex = "M_{T}",units = "GeV"))
    ...
    lg.append(ROOT.TLegend(0.2,0.6,0.6,0.8))
    lg[idx].SetName(sample+'_lg')
    lg[idx].AddEntry(h1[idx],"M(#nu#nu) = 0 GeV","pl")
    lg[idx].AddEntry(h2[idx],"M(#nu#nu) = 91.188 GeV","pl")
    lg[idx].AddEntry(h4[idx],"M(#nu#nu) = M(ll)","pl")

    c1.Clear()
    h2[idx].Draw('l hist')
    h1[idx].Draw('l hist same')
