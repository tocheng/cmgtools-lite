{
  ...
  TFile* fin = new TFile("test_GMCEtaWt_tightzpt50_puWeightsummer16_metfilter_unblind_mu_log_1pb.root");
  TFile* fout = new TFile("rhoratio_summer16.root", "recreate");

  gROOT->ProcessLine(".x tdrstyle.h");

  TH1D* hdt = (TH1D*)fin->Get("test_GMCEtaWt_tightzpt50_puWeightsummer16_metfilter_unblind_mu_log_1pb_rho_data_SingleEMU_Run2016Full_ReReco_v2");
  TH1D* hmc = (TH1D*)((THStack*)fin->Get("test_GMCEtaWt_tightzpt50_puWeightsummer16_metfilter_unblind_mu_log_1pb_rho_stack"))->GetHistogram();
  ...
  hratio_dt_mc->SetTitle("Rho reweighting: Normalized data/MC, 2016 data 36.81 fb^{-1}.");
  hratio_dt_mc->GetYaxis()->SetTitle("Data/MC");

  char name[10000];
  //sprintf(name, "gaus(0)+gaus(3)");
  sprintf(name, "gaus(0)+gaus(3)+gaus(6)");
  ...
  fratio_dt_mc->SetParameter(4,20);
  fratio_dt_mc->SetParameter(5,20);
  fratio_dt_mc->SetParameter(6,0.2);
  fratio_dt_mc->SetParameter(7,40);
  fratio_dt_mc->SetParameter(8,40);
  fratio_dt_mc->SetParLimits(0,0,2.0);
  fratio_dt_mc->SetParLimits(1,3,14);
  ...
  fratio_dt_mc->SetParLimits(6,0,2.0);
  fratio_dt_mc->SetParLimits(7,30,50);
  fratio_dt_mc->SetParLimits(8,10,50);
  ...
  sprintf(name, "      + %.2f #upoint Gaus(#rho; #mu=%.2f, #sigma=%.2f) ", fratio_dt_mc->GetParameter(6), fratio_dt_mc->GetParameter(7), fratio_dt_mc->GetParameter(8)); 
  lumipt->AddText(0.0,0.0, name);
  lumipt->Draw();

  plots->Print("rho_rewegiht_summer16.pdf");
  plots->SaveAs("rho_rewegiht_summer16.C");


  sprintf(name, 
          //"(%.3f*exp(-0.5*pow((x-%.3f)/%.3f,2))+%.3f*exp(-0.5*pow((x-%.3f)/%.3f,2)))",
          "(%.3f*TMath::Gaus(rho,%.3f,%.3f)+%.3f*TMath::Gaus(rho,%.3f,%.3f)+%.3f*TMath::Gaus(rho,%.3f,%.3f))",
          ...
          fratio_dt_mc->GetParameter(7),
          fratio_dt_mc->GetParameter(8)
         );
