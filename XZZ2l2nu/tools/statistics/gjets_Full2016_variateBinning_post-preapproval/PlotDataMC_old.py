...
def plotDataMC(sigType, channel,cat, parser):
    ...
    blindingCut = 100000 #300
    if(observable=="MET"):
       blindingCut = 100000 # 200
    ...
    r_full = TH1D('r_full', '',nbins,array('d',bins))
    r = TH1D('r', '',nbins,array('d',bins))
    ...
    for i in range(1,Shape_prefit['total_background'].GetXaxis().GetNbins()+1) :
        ...
        print i
        r_full.SetBinContent(i,1)
        r_full.SetBinError(i,max(errorUpsFull[i-1],errorDnsFull[i-1]))
        print max(errorUpsFull[i-1],errorDnsFull[i-1])
        r.SetBinContent(i,1)
        r.SetBinError(i,max(errorUps[i-1],errorDns[i-1]))
