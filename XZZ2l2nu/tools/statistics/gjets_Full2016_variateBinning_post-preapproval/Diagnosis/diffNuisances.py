...
for i in range(fpf_s.getSize()):
    ...
    if nuis_p == None:
        print 'nuisance parameter NOT present in the prefit result'
        #if not options.abs: continue
        row += [ "[%.2f, %.2f]" % (nuis_s.getMin(), nuis_s.getMax()) ]
        print 'min ',nuis_s.getMin(),'; max ',nuis_s.getMax()
        if not options.abs: continue
    else:
        print 'get best-fit value and uncertainty at prefit for this' 
        # nuisance parameter
        mean_p, sigma_p = (nuis_p.getVal(), nuis_p.getError())
        print 'mean_p ',mean_p,'; sigma_p ',sigma_p 
	          ...
	          if fit_name=='b':
	    	    nuis_p_i+=1
	      	    ...
	      	    hist_fit_b.SetBinError(nuis_p_i,nuis_x.getError()/sigma_p)
                    print 'b-only fit'
                    print nuis_p_i,' : ',nuis_x.getVal(),'+/-',nuis_x.getError()
...
if options.plotfile:
    ...
    canvas_nuis = ROOT.TCanvas("nuisancs", "nuisances", 800, 300)
    ...
    leg.Draw()
    dataset=args[0].strip('.root')
    dataset=dataset.strip('mlfit_')
    print dataset
    canvas_nuis.SaveAs('nuisance_'+dataset+'Asimov.pdf')

    fout.WriteTObject(canvas_nuis)
    canvas_pferrs = ROOT.TCanvas("post_fit_errs", "post_fit_errs", 800, 450)
