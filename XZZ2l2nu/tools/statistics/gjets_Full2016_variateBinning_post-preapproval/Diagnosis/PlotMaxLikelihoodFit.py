...
parser.add_option("--dataset", dest="Dataset",default='b-only',help="either b-only or s+b")
...
def ReadTemplates(MX,channel):
    ...
    inputfile = TFile("mlfit_"+dataset+".root","READ")
    ...
    for process in processes :
        ...
        c1.SaveAs(process+'_'+channel+'_m'+str(MX)+'_'+dataset+'Asimov.pdf')
        c1.SaveAs(process+'_'+channel+'_m'+str(MX)+'_'+dataset+'Asimov.png')
