...
int main(int argc, char** argv) {
  ...
  file_metshifts = new TFile("DYJetsToLL_M50_V4_doJetsCorrUseLepResPtErrSel8JetLepSigProtectV2_met_para_study.root");
  ...
  TFile* file_metshifts_data;
  file_metshifts_data = new TFile("SingleEMU_Run2015_16Dec_V4_doJetsCorrUseLepResPtErrSel8JetLepSigProtectV2_met_para_study.root");
...
*/     
    ...
    // signal protection
    if (doSignalProtection&&njet_corr>0){
      double min_jet_para = 1000000;
      for (int j=0; j<njet_corr; j++){
        int idx = jets_selected_index.at(j);
        if (idx<0) continue; // skim lepton
        double a_jet_para = jets_pt.at(j)*cos(jets_phi.at(j)-llnunu_l1_phi);
        if (a_jet_para<min_jet_para) min_jet_para = a_jet_para;
      }
      // cut on first jet para
      if (fabs(min_jet_para/llnunu_l1_pt)<0.4) {
        jets_selected_index.clear();
        njet_corr = 0;
      }
    }
