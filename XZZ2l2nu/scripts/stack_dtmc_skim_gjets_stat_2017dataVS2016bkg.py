...
paveText="#sqrt{s} = 13TeV L = "+"{:.4}".format(float(lumi))+"fb^{-1}"
...
for sample in sigSamples:
    ...
    sigPlotters[-1].setAlias('passMuHLT', '((llnunu_l1_l1_trigerob_HLTbit>>3&1)||(llnunu_l1_l2_trigerob_HLTbit>>3&1))');
...
Stack.addPlotter(Data, "data_obs", "Data 2017C, D", "data")
if muoneg: 
...
#Stack.drawStack('llnunu_mT', cuts, str(lumi*1000), nBins, mtMin, mtMax, titlex = "M_{T}", units = "GeV",output='mT',outDir=outdir,separateSignal=sepSig, blinding=Blind,blindingCut=200)

metnBins=50
metMin=0
metMax=500

Stack.drawStack('llnunu_MET', cuts, str(lumi*1000), metnBins, metMin, metMax, titlex = "E_{T}^{miss}", units = "GeV",output='MET',outDir=outdir,separateSignal=sepSig, blinding=Blind,blindingCut=200)
...
Stack.drawStack('llnunu_l2_pt_to_plot*sin(llnunu_l2_phi_to_plot-llnunu_l1_phi)', cuts, str(lumi*1000), 40, -200, 200.0, titlex = "MET_{#perp}", units = "GeV",output=tag+'met_perp',outDir=outdir,separateSignal=sepSig)
...
Stack.drawStack('llnunu_l1_l2_phi', cuts, str(lumi*1000), 32,-3.2,3.2, titlex = "#eta_{l2}", units = "",output='phil2',outDir=outdir,separateSignal=sepSig)
